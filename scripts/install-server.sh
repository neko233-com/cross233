...
VERSION="${CROSS233_VERSION:-v0.2.0}"
...
ExecStart=$INSTALL_DIR/cross233-server -bind \${CROSS233_BIND} -auth-key-file $DATA_DIR/cross233-auth.key -cert $DATA_DIR/cross233-cert.pem -key $DATA_DIR/cross233-key.pem
Restart=on-failure
RestartSec=3
...
printf '%s\n' "access key: sudo cat $DATA_DIR/cross233-auth.key"
